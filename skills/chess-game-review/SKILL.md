...
2. `.venv/bin/python scripts/analyze_game.py INPUT.pgn --output output/NAME` で解析。黒側なら `--orientation black`。依存関係はルートのREADME参照。保存済みJSONは入力・設定・エンジンが一致すれば再利用される。文章の修正だけなら再解析しない。
...
6. 各対局ディレクトリに `article.md` を作る。対局の概要、選んだ局面のSVG、実戦手と推奨手、狙いと相手の応手、次回の改善点を自然な日本語で書く。PVは短く、文章を支える範囲で示す。局面図は指す前、緑が推奨手、赤が実戦手。元のdraftと解析JSONを残す。
